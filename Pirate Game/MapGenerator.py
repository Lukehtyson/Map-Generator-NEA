...
'/Users/luketyson/Documents/Pirate Game/Background1.png'), 
pygame.image.load('/Users/luketyson/Documents/Pirate Game/Background2.png')]
...
play_button = Button(100, 200, pygame.image.load('/Users/luketyson/Documents/Pirate Game/start_button.png'), 8)
quit_button = Button(100, 400, pygame.image.load('/Users/luketyson/Documents/Pirate Game/quit_button.png'), 8)
...
def RenderFont(player_tile, background_xy, player_xy):
    ...
    font_pxy = font.render('player' + str(player_xy),  False, ("Black"))
...
def SpawnTreasure(player_xy):
    ...
    global gold
    if found:
        location = random.randint(0,len(cityareas)-1)
        found = False
    pygame.draw.rect(window, player_colour, (cityareas[location].x * tile_size - background_xy[0], cityareas[location].y * tile_size - background_xy[1], tile_size, tile_size))
    if abs(player_xy[0] - (cityareas[location].x) ) < 3 and abs(player_xy[1] - cityareas[location].y) < 3:
        gold = gold + 1
        found = True
        ...
        for x in range(border, map_width-border):
            for y in range(border, map_height-border):
                ...
                if Grid[x][y].surrounding_land < 7 and Grid[x][y].type == "Jungle":
                    Grid[x][y].type = "Beach" 
                if Grid[x][y].surrounding_land > 2 and Grid[x][y].surrounding_land < 6 and Grid[x][y].type == "Beach":
